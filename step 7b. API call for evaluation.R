...
load("Nytt datauttrekk/Leverandoer.Rdata")
...
save(station_data, file = "Nytt datauttrekk/station_data.Rdata")
######################################################
...
load("Nytt datauttrekk/Innsett.Rdata")
...
target_date <- as.Date("2021-06-10")
dates <- subset(dates, as.Date(prod_date) > target_date)
target_date <- as.Date("2023-04-17")
dates <- subset(dates, as.Date(prod_date) < target_date)
...
head(data)
# The data frame data is very large since it has several weather stations for each production day per innsett ID. This is because not all 
...
#Saving the results
write.csv(temperature_data, "Nytt datauttrekk/temperature_data.csv", row.names = FALSE)
save(temperature_data, file = "Nytt datauttrekk/Temperature.Rdata")
rm(results)
rm(xs)
...
head(humidity_data)
#Saving the results
save(humidity_data, file = "Nytt datauttrekk/Humidity.Rdata")
